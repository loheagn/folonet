...
fn attatch_xdp(iface: &str) -> Result<(Bpf, String), anyhow::Error> {
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let iface_list = ["lima0", "lima1", "eth0"];
    let mut bpf_list: Vec<(Bpf, String)> = iface_list
        .iter()
        .map(|iface| attatch_xdp(iface).unwrap())
        .collect();
    bpf_list.iter_mut().for_each(|(bpf, iface)| { 
        let opt = Opt{
            iface: iface.clone(),
        };
